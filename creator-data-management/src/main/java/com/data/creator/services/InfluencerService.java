...
public class InfluencerService {
    ...
    public List<Influencer> search(SearchRequest req) {
        ...
        // 排序
        Comparator<Influencer> comparator = Comparator.comparingLong(i -> Optional.ofNullable(i.getFollowersCount()).orElse(0));
        String sortBy = Optional.ofNullable(req.getSortBy()).orElse("followers_count");
        switch (sortBy) {
            case "avg_views":
                comparator = Comparator.comparingLong(i -> Optional.ofNullable(i.getAvgViews()).orElse(0));
                break;
    ...
    public Map<String, Object> compare(List<String> ids) {
        ...
        Map<String, Influencer> byId = influencers.stream()
                .collect(Collectors.toMap(i -> String.valueOf(i.getId()), x -> x));
        List<Influencer> selected = ids.stream()
                .map(byId::get)
    ...
    private List<Long> generateTrendLong(Integer base, int days, double fluct) {
